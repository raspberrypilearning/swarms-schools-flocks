...
```blocks3
when flag clicked
set rotation style [αριστερά-δεξιά v]
forever
...
set rotation style [αριστερά-δεξιά v]
forever
set [αριστερά-δεξιά v] to (pick random (-1) to (1))
if <(αριστερά-δεξιά) > (0)> then
...
forever
if <touching [θηρευτής v]> then
change [σκορ v] by [-10] // Επίλεξε αυτό για να μειώσεις το σκορ
change size by [-10] // Επίλεξε αυτό για να μειώσεις το μέγεθος
delete this clone // Επίλεξε αυτό για να αφαιρέσεις τον κλώνο
```
